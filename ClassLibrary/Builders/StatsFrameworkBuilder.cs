...
namespace WenRarityLibrary.Builders
{
    ...
    public class StatsFrameworkBuilder : FrameworkBuilder
    {
        ...
        private void StatsHandlerBlueprint(Collection collection, StatsContainer stats)
        {
            ...
            {
                var itemName = $"{collection.Name}{item}Rarity";

                sb.Append($"{OH(1, tabs)}// {item}");
            ...
            foreach (var item in stats.traitsIncluded)
            {
                sb.Append($"{OH(2, tabs)}var {item.ToLower()}Rarity = rimeContext.{collection.Name}{item}Rarity.ToList();");
            }
            ...
            foreach (var item in stats.traitsIncluded)
            {
                sb.Append($"{OH(2, tabs)}rarity.{item} = MH({item.ToLower()}Rarity.Where(i => i.{item} == item.{item}).FirstOrDefault().Count, size);");
            }
