using MarketWatcher.EntityFramework;
...
namespace MarketWatcher.SQL
{
    internal class RawSQLService
    {
		...
		//				conn.Close();
		//			}
		//			if (exists) DropTable(policy, collectionName);
		//		}
		//		catch (Exception ex)
		//		{
		//			ducky.Error("SQLService", "CheckRowsExist", ex.Message);
		//			throw;
		//		}
		//	}
		//}
		...
		public void Setfloor(string collectionName, double floor)
		{
			...
			//		using (var cmd = conn.CreateCommand())
			//		{
			//			cmd.CommandText = $"UPDATE [dbo].[JPGStoreCollections] SET [floor] = {floor} WHERE [collection_name] = '{collectionName}'";
			//			conn.Open();
			//			cmd.ExecuteNonQuery();
			//			conn.Close();
			//		}
